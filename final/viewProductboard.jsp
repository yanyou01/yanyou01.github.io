...
<body>
<a href="Productboard.jsp">寫寫留言</a><p>
<%
try {
//Step 1: 載入資料庫驅動程式 
...
//Step 4: 執行 SQL 指令, 若要操作記錄集, 需使用executeQuery, 才能傳回ResultSet	
           /*使用getRow()查詢筆數
		   sql = "SELECT * FROM `guestbook` WHERE `ProductNO`='" +session.getAttribute("ProductNO")+"';";  //算出共幾筆留言
           ...
           */
		   ...
		   //使用COUNT()查詢筆數
		   sql = "SELECT COUNT(*) FROM `guestbook` WHERE `ProductNO`='" +session.getAttribute("ProductNO")+"';"; //算出共幾筆留言
           ResultSet rs=con.createStatement().executeQuery(sql);
...
//Step 5: 顯示結果 
           ...
           //遞減排序, 讓最新的資料排在最前面
           sql="SELECT * FROM `guestbook` WHERE `ProductNO`='" +session.getAttribute("ProductNO")+"' ORDER BY `GBNO` DESC LIMIT ";
           sql+=start_record+",5";
...
//  逐筆顯示, 直到沒有資料(最多還是5筆)
           while(rs.next())
                {
			     out.println("產品:"+rs.getString(3)+"<br>");
				 out.println("留言內容:"+rs.getString(6)+"<br>");
                 out.println("留言主題:"+rs.getString(5)+"<br>");
                 out.println("訪客姓名:"+rs.getString(2)+"<br>");
                 out.println("E-mail:"+rs.getString(4)+"<br>");
                 out.println("留言時間:"+rs.getString(7)+"<br><hr>");
