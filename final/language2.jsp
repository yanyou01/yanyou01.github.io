...
//Step 4: 執行 SQL 指令, 若要操作記錄集, 需使用executeQuery, 才能傳回ResultSet	
           /*使用getRow()查詢筆數
		   sql="SELECT * FROM `productkist` WHERE `PDClass`='language'" ; //算出共幾筆留言
           ...
           */
		   //使用COUNT()查詢筆數
		   sql="SELECT COUNT(*) FROM `productlist`  WHERE `PDClass`='language'"; //算出共幾筆留言
           ResultSet rs=con.createStatement().executeQuery(sql);
...
//Step 5: 顯示結果 
           ...
           //遞減排序, 讓最新的資料排在最前面
           sql="SELECT * FROM `productlist` WHERE `PDClass`='language' ORDER BY `PDNO` ASC LIMIT ";
           sql+=start_record+",5";
...
//  逐筆顯示, 直到沒有資料(最多還是5筆)
          ...
          out.print("<h2>"+rs.getString(4)+"</h2>");
           out.print("<ul>");
			...
			out.print("<a href='language2.jsp?page="+i+"'>"+i+"</a>&nbsp;"); //&nbsp;html的空白
